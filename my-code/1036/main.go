...
func main() {
	fmt.Println(isEscapePossible([][]int{
		{0, 1},
		{1, 0},
	}, []int{0, 0}, []int{0, 2}))
}

// 1，两个都在外面，则都返回 1, 1*1=1
// 2，source在里面，target在外面，一个返回 0，一个返回 1, 0*1=0
// 3，source在外面，target在里面，一个返回 1，一个返回 0, 1*0=0
// 4，都在里面，在不同的包围圈内，都返回 0, 0*0=0
// 5，都在里面，在同一个包围圈内(能直接到达目标点)，都返回 2, 2*2=4
