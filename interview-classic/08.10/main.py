    ...
    def floodFill(self, image: List[List[int]], sr: int, sc: int, new_color: int) -> List[List[int]]:
        ...
        while dq:
            ...
            for move in {(1, 0), (-1, 0), (0, 1), (0, -1)}:
                ...
                if 0 <= _next[0] < row and 0 <= _next[1] < col and \
                        image[_next[0]][_next[1]] == old_color and _next not in path:
                    dq.appendleft(_next)
        return image


if __name__ == '__main__':
    print(Solution().floodFill([[1, 1, 1], [1, 1, 0], [1, 0, 1]], sr=1, sc=1, new_color=2))
